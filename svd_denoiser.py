...
parser.add_argument('--k', type=int, default=650, help='The number of required features')
parser.add_argument('--beta_1', type=float, default=0.3, help='coef for the filter')
parser.add_argument('--beta_2', type=float, default=2.5, help='coef for the filter')
parser.add_argument('--alpha', type=float, default=0.0, help='param for G^2N')
parser.add_argument('--eps', type=float, default=0.34, help='param for G^2N')
parser.add_argument('--gamma', type=float, default=1.6, help='weight for non-low frequency')
...
W=Variable(torch.eye(K,K).cuda(),requires_grad=True)
#W=Variable(torch.nn.init.uniform_(torch.randn(k,k),-np.sqrt(6. / (k+k)) ,np.sqrt(6. / (k+k) )).cuda(),requires_grad=True)
...
U,value,V=torch.svd_lowrank(norm_freq_matrix,q=K+100,niter=30)

user_init_embed,item_init_embed=U[:,:K],V[:,:K]




del value, D_u, D_i,norm_freq_matrix,U,V
gc.collect()
torch.cuda.empty_cache()
...
def test(rate_matrix):
	...
	'''
	project_norm_user=((W_u*user_init_embed).sum(1) / (user_init_embed**2).sum(1))
	project_norm_item=((W_p*item_init_embed).sum(1) / (item_init_embed**2).sum(1))
	project_norm_user = torch.nan_to_num(project_norm_user)
	project_norm_item = torch.nan_to_num(project_norm_item)


	W_u_pen = W_u - project_norm_user.unsqueeze(1)*user_init_embed

	W_p_pen = W_p - project_norm_item.unsqueeze(1)*item_init_embed

	###################################################################3

	#prediction = rate_matrix + (torch.sigmoid(W_u_pen.mm(W_p_pen.t())))*0.03

	prediction = rate_matrix + (user_init_embed.mm(W).mm(item_init_embed.mm(W).t()).sigmoid() * 0.5 + torch.sigmoid(W_u_pen.mm(W_p_pen.t())))*0.05

	#################################################
	'''

	#prediction = rate_matrix 
	#prediction = rate_matrix + (torch.sigmoid(W_u.mm(W_p.t())))*0.03

	#prediction = rate_matrix + user_init_embed.mm(W).mm(item_init_embed.mm(W).t()).sigmoid() * 0.2
	prediction = rate_matrix + (user_init_embed.mm(W).mm(item_init_embed.mm(W).t()).sigmoid() + torch.sigmoid(W_u.mm(W_p.t())))*train_weight
...
for i in range(80):
	...
	for j in range(0,epoch):
		...
		nega=torch.multinomial(1-freq_matrix[u],1,True).squeeze(1)

		
		user_embedding,pos_embedding,nega_embedding=user_init_embed[u].mm(W),item_init_embed[p].mm(W),item_init_embed[nega].mm(W)



		out=((user_embedding*pos_embedding).sum(1)-(user_embedding*nega_embedding).sum(1)).sigmoid()


		regu=0.01*(user_embedding**2+pos_embedding**2+nega_embedding**2).sum()


	
		

		weight_loss=torch.logdet(F.normalize(W,dim=0)+ 0.05 * torch.eye(K,K).cuda())/latent

		loss_1=(-torch.log(out).sum()+regu)/batch 
		...
		loss = 0.4 * loss_1 + align_loss+ uni_weight * uniform_loss -  1.0  * weight_loss
		#loss = 0.4 * loss_1 + align_loss+ 0.5 * uniform_loss 
	


		#loss=align_loss+ 5.0 * uniform_loss + spectral_loss + 5.0 * (spectral_user_distance + spectral_item_distance)/2
		loss.backward()
		with torch.no_grad():
			W-=lr*W.grad
			W_u-=lr*W_u.grad
